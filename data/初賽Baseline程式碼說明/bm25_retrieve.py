...
def BM25_retrieve(qs, source, corpus_dict):
    ...
    tokenized_corpus = [list(jieba_zh_tw.cut_for_search(doc)) for doc in filtered_corpus]  # 將每篇文檔進行分詞
    bm25 = BM25Okapi(tokenized_corpus)  # 使用BM25演算法建立檢索模型
    tokenized_query = list(jieba_zh_tw.cut_for_search(qs))  # 將查詢語句進行分詞
